...
namespace WarLight.Shared.AI.Prod.MakeOrders
{
    public class ExpandNormal : Expand
    {
        ...
        public ExpandNormal(BotMain bot)
        {
            ...
            var info = attackableTerritories.Select(territory => new KeyValuePair<TerritoryIDType, double>(territory.ID, territory.NumArmies.NumArmies));
            DataCollector.WriteStandingArmiesData(info);
